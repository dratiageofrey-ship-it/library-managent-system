...
def fines():
    ...
    try:
        ...
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        cur.execute('''SELECT f.fine_id, m.first_name, m.last_name, f.fine_amount, f.reason, f.status
                       FROM fines f JOIN members m ON f.member_id = m.member_id ORDER BY f.fine_id DESC''')
...
def reports():
    ...
    try:
        ...
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        cur.execute('''SELECT b.title, COUNT(l.loan_id) as borrow_count FROM books b LEFT JOIN loans l ON b.book_id = l.book_id
                       GROUP BY b.book_id, b.title ORDER BY borrow_count DESC LIMIT 10''')
